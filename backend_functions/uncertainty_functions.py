...
def plotFullCov(frac_cov_dict, var, cv, bins, xlow, xhigh, bincenters, x_ticks=None, save=False, axis_label=None, isrun3=False, pot=None): 
    ...
    cbar.ax.tick_params(labelsize=14)
    if pot: 
        cbar.set_label(label="$\\nu^{2}$ / "+pot+"$^{2}$", fontsize=18, labelpad=7)
    ...
    if axis_label is not None: 
        plt.xlabel(axis_label, fontsize=18, labelpad=7)
        plt.ylabel(axis_label, fontsize=18, labelpad=7)
    else: 
        plt.xlabel(var, fontsize=18, labelpad=7)
        plt.ylabel(var, fontsize=18, labelpad=7)

    plt.title('Fractional Covariance', fontsize=18)
    ...
    if axis_label is not None: 
        plt.xlabel(axis_label, fontsize=18, labelpad=8)
        plt.ylabel(axis_label, fontsize=18, labelpad=7)
    else: 
        plt.xlabel(var, fontsize=18, labelpad=7)
        plt.ylabel(var, fontsize=18, labelpad=7)
    ...
    for i in range(len(bins)-1): 
        for j in range(len(bins)-1): 
            ...
            if not np.isnan(abs_cov[i][j]):
                if round(abs_cov[i][j],2)>100: 
                    plt.text(bincenters[i], bincenters[j], round(abs_cov[i][j], 2), 
                        color='white', ha="center", va="center", fontsize=12, fontweight='bold') # 10.5, 10, 12
                else: 
                    plt.text(bincenters[i], bincenters[j], round(abs_cov[i][j], 2), 
                        color='black', ha="center", va="center", fontsize=13, fontweight='bold') # 11, 10.5, 13
    
    if save: 
        plt.savefig("/Users/abarnard/Desktop/analysis_plots/full_cov/Absolute_Covariance_"+var+".pdf", transparent=False, bbox_inches='tight') 
    plt.show()
